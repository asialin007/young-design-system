...
| 页面 | 文件 | 说明 |
|------|------|------|
| 手机号登录 | `login-phone.html` | 输入手机号 → 获取验证码；可跳转「账号密码登录」「注册新账号」 |
| 账号密码登录 | `login-password.html` | 账号 + 密码登录；可跳转「手机号登录」「注册新账号」 |
| 手机注册 | `register-phone.html` | 输入手机号 → 下一步（验证码/设置密码）；可跳转「去登录」 |

**入口建议**：以 `login-phone.html` 为默认登录入口，三页之间通过底部链接互相跳转。
