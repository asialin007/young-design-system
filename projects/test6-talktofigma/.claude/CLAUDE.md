...
**⚠️ 重要：移动端应该自适应设备宽度，而不是固定宽度！**

**所有生成的 HTML 页面必须遵循移动端适配规范，确保：**
- ✅ 禁止水平滚动
- ✅ **限制最小宽度为 375px**（保证小屏幕可读性）
- ✅ **宽度 100%，根据设备宽度自适应**（真正的移动端响应式）
- ✅ 在所有设备上全屏显示
...
/* ========== 移动端容器（必须） ========== */
.mobile-container {
  width: 100%;  /* 100% 宽度，根据设备自适应 */
  min-width: 375px;  /* 最小宽度 375px，保证小屏幕可读性 */
  overflow-x: hidden;  /* 禁止水平滚动 */
  position: relative;
}
```
...
| **容器宽度** | `width: 100%` | 根据设备宽度自适应 |
| **容器最小宽度** | `min-width: 375px` | 保证小屏幕可读性 |
| **禁止居中** | 不使用 `margin: 0 auto` | 让页面占满整个设备宽度 |
| **字体平滑** | `-webkit-font-smoothing: antialiased` | 提升字体渲染质量 |

**⚠️ 重要说明：**
- ❌ **禁止使用** `max-width: 375px` + `margin: 0 auto`（这是固定宽度，不是真正的移动端）
- ✅ **必须使用** `width: 100%` + `min-width: 375px`（这是自适应宽度，真正的移动端响应式）

### 常见错误对照表

| ❌ 错误做法 | ✅ 正确做法 | 说明 |
|-----------|-----------|------|
| 没有 `mobile-container` | 所有内容包裹在 `.mobile-container` 中 | 确保移动端适配 |
| 没有 `overflow-x: hidden` | 添加 `overflow-x: hidden` | 禁止水平滚动 |
| **使用 `max-width: 375px`** | **使用 `min-width: 375px`** | **保证最小宽度，而不是固定宽度** |
| **使用 `margin: 0 auto` 居中** | **不使用居中** | **让页面占满整个设备宽度** |
| 没有 `viewport` meta 标签 | 添加完整的 `viewport` 设置 | 移动端视口适配 |
| 内容直接放在 `body` 中 | 内容放在 `.mobile-container` 中 | 确保移动端适配 |
...
**⚠️ 关键理解：**
- 桌面浏览器和移动设备都是 **100% 宽度自适应**
- 差别在于：桌面浏览器宽度更大，但页面会占满整个宽度
- 真正的移动端响应式 = **自适应所有设备宽度**，而不是固定 375px

### 测试检查清单

在生成 HTML 页面后，必须检查：

- [ ] **页面在桌面浏览器中是否占满整个宽度**（❌ 不应该居中显示 375px）
- [ ] **页面宽度是否自适应**（✅ 应该根据浏览器宽度自适应）
- [ ] **是否使用了 `min-width: 375px`**（✅ 保证最小宽度）
- [ ] **是否没有使用 `max-width: 375px`**（❌ 禁止固定宽度）
- [ ] **是否没有使用 `margin: 0 auto`**（❌ 禁止居中）
- [ ] 页面是否可以左右滚动（❌ 不应该可以）
- [ ] 页面是否可以上下滚动（❌ 不应该有滚动条）
...
<!-- 带图标的按钮（黑色按钮用白色图标）-->
<button class="yds-button yds-button--solid-dark yds-button--black yds-button--xl">
  <img src="../../../icons/y_icon_line_edit_add.svg"
       class="yds-icon"
...
```bash
# 搜索特定关键词的图标
ls ../../../icons/ | grep -i "返回关键词"

# 常用搜索示例
ls ../../../icons/ | grep -i "arrow_left"    # 返回图标
ls ../../../icons/ | grep -i "visible"      # 可见性图标
ls ../../../icons/ | grep -i "password"     # 密码图标
ls ../../../icons/ | grep -i "close"        # 关闭图标
ls ../../../icons/ | grep -i "search"       # 搜索图标
```
...
toggle.addEventListener('click', () => {
  ...
  toggle.innerHTML = visible
    ? '<img src="../../../icons/y_icon_line_edit_passwordvisible.svg" alt="隐藏密码">'
    : '<img src="../../../icons/y_icon_line_edit_invisible.svg" alt="显示密码">';
});
</script>
```
...
```html
<!-- ❌ 错误：使用不存在的图标文件 -->
<img src="../../../icons/y_icon_line_generality_end.svg" alt="显示密码">
...
### 引用文件检查
- [ ] 引用 YDS CSS：`../../../css/index.css`
- [ ] 图标引用使用相对路径：`../../../icons/xxx.svg`
- [ ] **禁止使用绝对路径（⚠️ 常见错误！）**
- [ ] JavaScript 中的图标路径也使用相对路径
...
.history-tag,
.recommend-tag {
  border-radius: var(--yds-radius-full);  /* 圆角 */
}
```
...
.history-tag,
.recommend-tag {
  border-radius: var(--yds-radius-none);  /* 直角 0px */
}
```
...
/* ❌ 错误：商品价格使用错误色 */
.current-price {
  color: var(--yds-text-danger-default);  /* 红色 #E93E3E */
}
```
...
/* ✅ 正确：商品价格使用品牌色 */
.current-price {
  color: var(--yds-text-brand-default);  /* 品牌色 #E52EC5 */
}
```
...
| 用途 | 错误颜色 | 正确颜色 | 颜色值 |
|------|---------|---------|--------|
| **商品价格** | ❌ `var(--yds-text-danger-default)` | ✅ `var(--yds-text-brand-default)` | #E52EC5（粉紫色） |
| **错误提示** | ✅ `var(--yds-text-danger-default)` | ✅ `var(--yds-text-danger-default)` | #E93E3E（红色） |
| **成功提示** | ✅ `var(--yds-text-positive-default)` | ✅ `var(--yds-text-positive-default)` | #2AA764（绿色） |

### 3. 图标使用规范

**⚠️ 重要：操作类按钮必须使用平面填充图标（planarity）**

| 位置 | 错误图标 | 正确图标 | 文件名 |
|------|---------|---------|--------|
| **删除历史记录** | ❌ 线性删除图标 | ✅ 平面填充删除图标 | `y_icon_planarity_edit_delete.svg` |
| **清空按钮** | ❌ 线性关闭图标 | ✅ 平面填充关闭图标 | `y_icon_planarity_edit_close.svg` |
| **拍立购** | ❌ 相机切换图标 | ✅ 图片切换图标 | `y_icon_line_edit_picture_switch.svg` |
| **换一批** | ❌ 搜索图标 | ✅ 刷新图标 | `y_icon_line_edit_refresh.svg` |

**图标类型选择规则：**

| 场景 | 图标类型 | 示例 |
|------|---------|------|
| **操作按钮（删除、清空等）** | ✅ 平面填充图标（planarity） | `y_icon_planarity_edit_delete.svg` |
| **功能入口（搜索、切换等）** | ✅ 线性图标（line） | `y_icon_line_edit_search.svg` |

**完整示例：**
```html
<!-- ✅ 正确：删除图标使用平面填充 -->
<img src="../../../icons/y_icon_planarity_edit_delete.svg"
     class="delete-icon"
     alt="删除">

<!-- ✅ 正确：清空图标使用平面填充 -->
<img src="../../../icons/y_icon_planarity_edit_close.svg"
     class="clear-icon"
     alt="清空">

<!-- ✅ 正确：拍立购使用图片切换图标 -->
<img src="../../../icons/y_icon_line_edit_picture_switch.svg"
     class="camera-icon"
     alt="拍立购">

<!-- ✅ 正确：换一批使用刷新图标 -->
<img src="../../../icons/y_icon_line_edit_refresh.svg"
     class="refresh-icon"
     alt="换一批">
```
...
**✅ 正确：弹出 YDS Dialog 确认对话框**
```javascript
// ✅ 正确：弹出确认对话框
function clearHistory() {
  const historyTags = document.getElementById('history-tags');

  if (historyTags.children.length === 0) {
    showToast('暂无历史记录');
    return;
  }
...
function confirmClearHistory() {
  const historyTags = document.getElementById('history-tags');
  // 清空历史记录
  historyTags.innerHTML = '<div class="history-tag" style="background: transparent; color: var(--yds-text-default-tertiary);">暂无历史记录</div>';
  showToast('历史记录已清空');
  // 关闭对话框
  closeDialog();
}
...
**HTML 结构（使用 YDS Dialog 组件）：**
```html
<!-- 确认对话框（使用 YDS Dialog 组件）-->
<div class="yds-dialog-overlay" id="confirm-dialog" style="display: none;">
  <div class="yds-dialog">
    <div class="yds-dialog__content">
      <p class="yds-dialog__body-text">确定清空所有历史记录吗？</p>
    </div>
    <div class="yds-dialog__buttons yds-dialog__buttons--horizontal">
      <button class="yds-dialog__button yds-dialog__button--secondary" onclick="closeDialog()">取消</button>
      <button class="yds-dialog__button yds-dialog__button--primary" onclick="confirmClearHistory()">确定</button>
    </div>
  </div>
</div>
```
...
**⚠️ 重要：Sticky 定位需要明确的滚动容器才能生效！**

**核心规则：**
- `position: sticky` 需要在**有滚动条的容器**中才能工作
- 必须**明确设置 `overflow-y: auto`** 在滚动容器上
- **禁止**在 sticky 元素的任何祖先容器上设置 `overflow: hidden`

#### 正确的页面基础样式

```css
/* ✅ 正确：明确设置 overflow-y: auto */
html {
  width: 100%;
  height: 100%;
  overflow-x: hidden;
...
.sort-bar {
  ...
  top: 60px;  /* 根据搜索框高度调整 */
  z-index: 99;
  border-bottom: 1px solid var(--yds-border-default-tertiary);
}
```
...
| 项目 | 设置 | 说明 |
|------|------|------|
| **滚动容器** | `overflow-y: auto` | **必须**在 body 或 mobile-container 上设置 |
| **position** | `sticky` | 启用粘性定位 |
| **top** | `0` 或具体数值 | 距离视口顶部的偏移 |
| **z-index** | `99` ~ `100` | 确保在其他元素之上 |
| **background-color** | 不透明颜色 | 必须设置，否则透明会导致问题 |
...
│  视口（Viewport）              │
│  ┌───────────────────────────┐  │
│  │ .search-header (sticky) │  │ ← top: 0 时固定在这里
│  ├───────────────────────────┤  │
│  │ 滚动内容...              │  │
│  │                         │  │
│  │ .sort-bar (sticky)      │  │ ← top: 60px 时固定在这里
│  ├───────────────────────────┤  │
│  │ 更多内容...              │  │
│  │                         │  │
└─────────────────────────────┘  └
...
| 问题 | 原因 | 解决方案 |
|------|------|----------|
| **Sticky 不生效** | 缺少 `overflow-y: auto` | 在 body/mobile-container 上添加 |
| **Sticky 不生效** | 祖先容器有 `overflow: hidden` | 移除或改用 `overflow-y: auto` |
| **Sticky 元素闪烁** | `z-index` 层级冲突 | 增加 `z-index` 值 |
| **Sticky 定位错误** | `top` 值设置不当 | 根据实际内容调整 `top` 值 |

### 7. 导航栏布局规范（⚠️ 2025-02-04 更新）

**⚠️ 重要：导航栏（搜索框、排序栏）作为顶层悬浮元素，必须拉满屏幕宽度！**
...
<div class="yds-grid">
  <div class="yds-grid__cell yds-grid__cell--12">
    <div class="yds-grid__cell-content">
      <div class="search-header">...</div>  <!-- 导航栏被栅格约束 -->
    </div>
  </div>
</div>
```

**✅ 正确：导航栏在栅格系统外面，拉满屏幕宽度**
```html
<!-- ✅ 正确：导航栏独立于栅格系统 -->
<div class="mobile-container">
  <div class="search-header">...</div>  <!-- 导航栏在栅格外 -->
  <div class="sort-bar">...</div>       <!-- 导航栏在栅格外 -->
  <div class="yds-grid">               <!-- 内容区域使用栅格 -->
    <div class="yds-grid__cell yds-grid__cell--12">
      <div class="yds-grid__cell-content">
        <!-- 商品列表等内容 -->
      </div>
...
/* ✅ 正确：导航栏拉满屏幕宽度 */
.search-header {
  padding: var(--yds-spacing-12) var(--yds-spacing-8);  /* 左右8px内边距 */
  position: relative;
  z-index: 100;
  width: 100%;   /* ✅ 拉满屏幕宽度 */
  left: 0;
...
| 样式 | 值 | 说明 |
|------|---|------|
| **padding** | `var(--yds-spacing-12) var(--yds-spacing-8)` | 上下12px，左右8px（内容不贴边）|
| **width** | `100%` | 拉满屏幕宽度 |
| **left/right** | `0` | 从左到右完全占满 |
| **position** | `relative`（默认）/ `fixed`（悬浮） | 支持固定悬浮 |
...
/* ✅ 正确：添加上边距 */
.product-list {
  padding: var(--yds-spacing-8) 0 0 0;  /* 上边距8px */
}
...
```
┌─────────────────────────────────────┐
│ .mobile-container (100%宽度)        │
│ ┌─────────────────────────────────┐ │
│ │ 搜索框（100%宽度，左右padding 8px）│ │ ← 拉满屏幕
│ ├─────────────────────────────────┤ │
│ │ 排序栏（100%宽度，左右padding 8px）│ │ ← 拉满屏幕
│ ├─────────────────────────────────┤ │
│ │ ↓ 8px 间隔                     │ │
│ │ ┌─────────────────────────────┐ │ │
│ │ │ 商品列表（栅格，左右8px）   │ │ │ ← 内容有间距
...
**布局原则：**
1. ✅ 导航栏（搜索框、排序栏）独立于栅格系统外
2. ✅ 导航栏宽度 100%，拉满屏幕
3. ✅ 导航栏使用 `padding: 12px 8px`（上下12px，左右8px）
4. ✅ 内容区域使用栅格系统，有左右 8px 间距
5. ✅ 内容区域添加 `padding-top: 8px`，与导航栏保持间隔

### 8. 搜索页面设计规范总结

| 规范项 | 错误做法 | 正确做法 |
|--------|---------|---------|
| **标签圆角** | `border-radius: var(--yds-radius-full)` | `border-radius: var(--yds-radius-none)` |
| **搜索框圆角** | `border-radius: var(--yds-radius-full)` | `border-radius: var(--yds-radius-none)` |
| **商品价格颜色** | `var(--yds-text-danger-default)` | `var(--yds-text-brand-default)` |
| **商品图尺寸** | 100x133px（长图） | 100x100px（方图） |
| **删除/清空图标** | 线性图标（line） | 平面填充图标（planarity） |
| **删除确认** | 直接清空 | 弹出 YDS Dialog 确认 |
| **导航栏宽度** | 放在栅格系统内 | 独立于栅格系统，100%宽度 |
| **导航栏内边距** | `padding: 12px 0` | `padding: 12px 8px` |
| **内容间隔** | 无上边距 | `padding-top: 8px` |
| **Sticky 定位** | 未设置或设置错误 | `position: sticky; top: 0; z-index: 100` |

### 9. 搜索页面常见错误对照表

| ❌ 错误做法 | ✅ 正确做法 | 说明 |
|-----------|-----------|------|
| 历史标签使用圆角 | 历史标签使用直角 | `border-radius: var(--yds-radius-none)` |
| 推荐标签使用圆角 | 推荐标签使用直角 | `border-radius: var(--yds-radius-none)` |
| 搜索框使用圆角 | 搜索框使用直角 | `border-radius: var(--yds-radius-none)` |
| 商品价格使用红色 | 商品价格使用品牌色 | `color: var(--yds-text-brand-default)` |
| 商品图使用长图 | 商品图使用方图 | `height: 100px`（1:1） |
| 删除图标使用线性图标 | 删除图标使用平面填充图标 | `y_icon_planarity_edit_delete.svg` |
| 清空图标使用线性图标 | 清空图标使用平面填充图标 | `y_icon_planarity_edit_close.svg` |
| 换一批使用搜索图标 | 换一批使用刷新图标 | `y_icon_line_edit_refresh.svg` |
| 拍立购使用相机图标 | 拍立购使用图片切换图标 | `y_icon_line_edit_picture_switch.svg` |
| 删除历史记录直接清空 | 弹出确认对话框 | 使用 YDS Dialog 组件 |
| 搜索框/排序栏在栅格内 | 搜索框/排序栏在栅格外 | 独立于栅格系统 |
| 导航栏未设置100%宽度 | 导航栏拉满屏幕宽度 | `width: 100%; left: 0; right: 0;` |
| 导航栏无左右padding | 导航栏有左右8px padding | `padding: 12px 8px` |
| 内容区域无上边距 | 内容区域有上边距8px | `padding-top: 8px` |
| 搜索框未设置 sticky | 搜索框设置 sticky | `position: sticky; top: 0` |
...
23. ❌ **禁止使用绝对路径引用CSS和图标** → 必须使用相对路径 `../../../css/` 和 `../../../icons/`

**搜索页面特定禁止事项：**
24. ❌ **禁止搜索页面标签使用圆角** → 必须使用直角 `border-radius: var(--yds-radius-none)`
25. ❌ **禁止搜索框使用圆角** → 必须使用直角 `border-radius: var(--yds-radius-none)`
26. ❌ **禁止商品价格使用错误色** → 必须使用品牌色 `var(--yds-text-brand-default)`
27. ❌ **禁止商品图使用长图** → 必须使用方图 100x100px（1:1）
28. ❌ **禁止删除/清空图标使用线性图标** → 必须使用平面填充图标（planarity）
29. ❌ **禁止破坏性操作直接执行** → 必须弹出 YDS Dialog 确认对话框
30. ❌ **禁止搜索框和排序栏缺少 sticky 定位** → 必须设置 `position: sticky; top: 0; z-index: 100`
31. ❌ **禁止缺少 `overflow-y: auto`** → Sticky 定位需要明确的滚动容器，必须在 body/mobile-container 上设置 `overflow-y: auto`
32. ❌ **禁止导航栏放在栅格系统内** → 导航栏必须独立于栅格系统，拉满屏幕宽度
33. ❌ **禁止导航栏未设置100%宽度** → 必须设置 `width: 100%; left: 0; right: 0;`
34. ❌ **禁止导航栏缺少左右padding** → 必须设置 `padding: 12px 8px`（上下12px，左右8px）
35. ❌ **禁止内容区域缺少上边距** → 必须设置 `padding-top: 8px`，与导航栏保持间隔
...
- **颜色系统**：`../../../css/colors.css`
- **按钮组件**：`../../../css/button.css`
- **复选框组件**：`../../../css/checkbox.css`
- **单选框组件**：`../../../css/radio.css`
- **对话框组件**：`../../../css/dialog.css`
- **栅格系统**：`../../../css/grid.css`
- **导航栏组件**：`../../../css/navbar.css`
- **标签栏组件**：`../../../css/tabbar.css`
...
- v12.0 - 添加"搜索页面设计规范"：标签直角、商品价格品牌色、商品图方图、图标类型选择、删除确认对话框、Sticky 定位
- v13.0 - 添加"导航栏布局规范"：导航栏独立于栅格系统、100%宽度、左右8px padding；修正搜索框圆角规范（改为直角）
- v14.0 - 添加"CSS和图标引用相对路径规范"：所有HTML文件必须使用相对路径引用CSS和图标，禁止使用绝对路径
